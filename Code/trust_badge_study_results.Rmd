...
Check survey completion time. Remove participants who took less than 90 seconds to complete, and remove one participant who had a missing value for age group.

```{r}
survey <- survey |>
  mutate(Duration = as.numeric(`Duration (in seconds)`)) |>
  filter(`Duration (in seconds)` > 90 | !is.na(Q46))
```
...
survey <- survey |>
  mutate(num_online_purchase = as.numeric(factor(num_online_purchase, levels = c("1", "2", "3", "4", "5 or more"))),
         age_group = factor(age_group, levels = c("Under 18", "18-24 years old", "25-34 years old", "35-44 years old", "45-54 years old", "55-64 years old", "65+ years old")))
```

Check for missing values in variables of interest.

```{r}
survey <- survey |>
  select(purchase, helpful, review_trust, reviewer_trust, platform_trust, QID11, QID12, education, ethnicity, num_online_purchase, age_group, sex, income, Treat)

lapply(survey, function(x) sum(is.na(x)))
```
...
Online purchase breakdown.

```{r}
survey |>
  group_by(Treat) |>
  summarize(mean_online_purchase = mean(num_online_purchase))
```
